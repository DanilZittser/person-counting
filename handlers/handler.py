...
class Handler(ABC):
    ...
    def on_start(self, *args, **kwargs) -> Any:
        ...
        Общая рекомендация следующая: в конструкторе оставляем инициализацию только простых типов (int, float, str).
        Идея в том, чтобы сократить объём копирования при запуске хендлера в отдельном fork-процессе.
    ...
    def on_exit(self, *args, **kwargs) -> None:
        """Метод, вызывающийся после завершения работы компонента. Должен освобождать инициализированные ресурсы."""
        pass
